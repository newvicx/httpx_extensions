...
def test_proxies_parameter(proxies, expected_proxies):
    client = httpx_extensions.ExClient(proxies=proxies)

    for proxy_key, url in expected_proxies:
        pattern = URLPattern(proxy_key)
        assert pattern in client._mounts
        proxy = client._mounts[pattern]
        assert isinstance(proxy, httpx_extensions.AsyncHTTPTransportMixin)
        assert isinstance(proxy._pool, httpx_extensions.httpcore.proxy.AsyncHTTPProxyMixin)
...
def test_socks_proxy():
    ...
    async_client = httpx_extensions.ExClient(proxies="socks5://localhost/")
    async_transport = async_client._transport_for_url(url)
    assert isinstance(async_transport, httpx_extensions.AsyncHTTPTransportMixin)
    assert isinstance(async_transport._pool, httpx_extensions.httpcore.socks.AsyncSOCKSProxyMixin)
...
def test_transport_for_request(url, proxies, expected):
    ...
    if expected is None:
        ...
        assert isinstance(transport, httpx_extensions.AsyncHTTPTransportMixin)
        assert isinstance(transport._pool, httpx_extensions.httpcore.proxy.AsyncHTTPProxyMixin)
...
async def test_async_proxy_close():
    try:
        client = httpx_extensions.ExClient(proxies={"https://": PROXY_URL})
...
def test_unsupported_proxy_scheme():
    with pytest.raises(ValueError):
        httpx_extensions.ExClient(proxies="ftp://127.0.0.1")
...
def test_for_deprecated_proxy_params(proxies, is_valid):
    if not is_valid:
        with pytest.raises(ValueError):
            httpx_extensions.ExClient(proxies=proxies)
    else:
        httpx_extensions.ExClient(proxies=proxies)
