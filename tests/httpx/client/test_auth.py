...
class App:
    ...
    def __call__(self, request: httpx.Request) -> httpx_extensions.ResponseMixin:
        headers = {"www-authenticate": self.auth_header} if self.auth_header else {}
        data = {"auth": request.headers.get("Authorization")}
        return httpx_extensions.ResponseMixin(self.status_code, headers=headers, json=data)
...
class DigestApp:
    ...
    def challenge_send(self, request: httpx.Request) -> httpx_extensions.ResponseMixin:
...
async def test_basic_auth_with_stream() -> None:
    ...
    async with httpx_extensions.ExClient(
        transport=httpx_extensions.mock.MockTransport(app), auth=auth
    ) as client:
...
async def test_trust_env_auth() -> None:
    ...
    async with httpx_extensions.ExClient(
        transport=httpx_extensions.mock.MockTransport(app), trust_env=False
    ) as client:
        response = await client.get(url)
    ...
    async with httpx_extensions.ExClient(
        transport=httpx_extensions.mock.MockTransport(app), trust_env=True
    ) as client:
        response = await client.get(url)
...
async def test_digest_auth_unavailable_streaming_body():
    ...
    async with httpx_extensions.ExClient(transport=ConsumeBodyTransport(app)) as client:
