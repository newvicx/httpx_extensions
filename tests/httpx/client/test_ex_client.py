...
async def test_context_managed_transport():
    class Transport(httpx_extensions.MockTransport):
        def __init__(self):
            self.events = []
    ...
    transport = Transport()
    async with httpx_extensions.ExClient(transport=transport):
        pass
...
def hello_world(request):
    return httpx_extensions.models.ResponseMixin(200, text="Hello, world!")

# adjust
@pytest.mark.usefixtures("async_environment")
async def test_client_closed_state_using_implicit_open():
    client = httpx_extensions.ExClient(transport=httpx_extensions.MockTransport(hello_world))
...
async def test_client_closed_state_using_with_block():
    async with httpx_extensions.ExClient(transport=httpx_extensions.MockTransport(hello_world)) as client:
        assert not client.is_closed
        await client.get("http://example.com")
...
def unmounted(request: httpx.Request) -> httpx_extensions.models.ResponseMixin:
    data = {"app": "unmounted"}
    return httpx_extensions.models.ResponseMixin(200, json=data)


def mounted(request: httpx.Request) -> httpx_extensions.models.ResponseMixin:
    data = {"app": "mounted"}
    return httpx_extensions.models.ResponseMixin(200, json=data)
...
async def test_mounted_transport():
    transport = httpx_extensions.MockTransport(unmounted)
    mounts = {"custom://": httpx_extensions.MockTransport(mounted)}

    async with httpx_extensions.ExClient(transport=transport, mounts=mounts) as client:
...
async def test_async_mock_transport():
    ...
    transport = httpx_extensions.MockTransport(hello_world)

    async with httpx_extensions.ExClient(transport=transport) as client:
        response = await client.get("https://www.example.com")
        assert response.status_code == 200
        assert response.text == "Hello, world!"
